...
silhouette_method <- k_means_data %>%
...
career_growth <- degrees_labeled %>%
    ggplot(aes(x = Starting.Median.Salary, Mid.Career.Median.Salary, color = factor(clusters))) +
    geom_point() +
    ggtitle('Salary Change') +
    xlab('Starting Median Salary') +
    ylab('Middle Career Median Salary') +
